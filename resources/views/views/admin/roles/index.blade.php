...
@section('content')
    ...
    <div class="card card-default">
        ...
        <div class="card-body table-responsive">
            <table id="roleTable" class="table table-bordered table-striped {{ count($roles) > 0 ? 'datatable' : '' }} dt-select">
                <thead>
                    <tr>
                    ...
                    @if (count($roles) > 0)
                        @foreach ($roles as $role)
                            <tr data-entry-id="{{ $role->id }}">
                                ...
                                <td>
                                    <a href="{{ route('admin.roles.edit',[$role->id]) }}" class="btn btn-info">@lang('global.app_edit')</a>
                                    {!! Form::open(array(
